...
app.get('/', async ({ query }, response) => {
	response.sendFile('index.html', { root: './public/src/html/' });
});

app.get('/success', async ({ query }, response) => {
	const 
		{ code } = query,
		steamSchema = require("./public/src/db/schemas/steamSchema");
		...
		try {
			...
			if(oauthResult.status === 200) {
				...
				if(!steamQuery) {
					...
					if(!steamAccount) {
						response.redirect('nolinkedsteam');
					} else {
			...
			response.sendFile('success.html', { root: './public/src/html/' })
		} catch (error) { 
			console.log(error);
			return response.redirect('error');
		}
...
app.get("/alreadylinked", ({ query }, response) => {
	response.sendFile('alreadyLinked.html', { root: './public/src/html/' });
})
app.get("/nolinkedsteam", ({ query }, response) => {
	response.sendFile('nolinkedsteam.html', { root: './public/src/html/' });
})
app.get("/error", ({ query }, response) => {
	response.sendFile('error.html', { root: './public/src/html/' });
})
